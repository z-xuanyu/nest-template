...
export default function useModal(props, { emit, attrs }: SetupContext) {
  ...
  function drag(oDrag, handle) {
    ...
    maxRef.value.onclick = function () {
      ...
      modalBody.value.style.height =
        parseFloat(oDrag.style.height) - headerHeight - footerHeight + 'px'
  ...
  const initWin = () => {
    ...
    dragRef.value.style.left =
      (document.documentElement.clientWidth - dragRef.value.offsetWidth) / 2 + 'px'
    dragRef.value.style.top = props.centered
      ? (document.documentElement.clientHeight - dragRef.value.offsetHeight) / 2 + 'px'
      : '100px'
