<template>
  <a-modal
    ...
    :title="Number.isInteger(fields._id) ? '编辑菜单' : '新增菜单'"
    ...
    <a-form ref="formRef" :model="modelRef" :label-col="labelCol" :wrapper-col="wrapperCol">
      <a-form-item label="类别" :rules="rules.type" name="type">
        <a-select
          ...
          :disabled="Number.isInteger(fields._id)"
          placeholder="请选择类型"
        >
      ...
      >
        <a-select
          ...
          placeholder="请选择模块"
        >
          <a-select-option v-for="item in moduleList" :key="item._id" :value="item._id">
            {{ item.name }}
          </a-select-option>
        </a-select>
      </a-form-item>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    if (props.fields.parentId) {
      ...
      modelRef.type = props.fields.parentId == null ? 1 : 2
    }
    ...
    onMounted(async () => {
      // 获取模块列表
      const res:any  = await getAdminAccessModule()
      state.moduleList = res
    })
    ...
    const handleOk = (e) => {
      ...
      formRef.value
        .validate()
        .then(async () => {
          const _id = props.fields._id
          const params = toRaw(modelRef)
          _id && delete params.type
          await (_id ? patchAdminAccess(_id, params) : postAdminAccess(params)).finally(
